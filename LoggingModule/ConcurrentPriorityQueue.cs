...
namespace LoggingModule
{
    ...
    public class ConcurrentPriorityQueue<TKey, TValue> :
        IProducerConsumerCollection<KeyValuePair<TKey, TValue>>, IProducerConsumerCollection<TValue>
        where TKey : class, IComparable<TKey>
        where TValue : LogMessage
    {
        ...
        TValue[] IProducerConsumerCollection<TValue>.ToArray()
        {
            throw new NotImplementedException();
        }
        ...
        {
            Enqueue(item.Severity as TKey, item);
            return true;
        }
        ...
        public bool TryTake(out TValue item)
        {
            ...
            //keyValuePair.Key = item.Severity;
            TryDequeue(out keyValuePair);
        ...
        {
            var arr = ToArray().OrderBy(x => x.Key);
            var selected = arr.Select(x => x.Value);
